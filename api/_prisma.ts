...
try {
  // Try to import real Prisma client at runtime (works only if generation was allowed)
  const mod: any = await import("@prisma/client").catch(() => null);
  const PrismaClientCtor: any = mod?.PrismaClient || mod?.default?.PrismaClient;
        ...
        if (opts?.where?.AND?.length) {
          arr = arr.filter((o) =>
            opts.where.AND.every((f: any) => {
              ...
              const v = cond[k]?.contains || "";
              const hay = `${o.title}\n${o.description}`.toLowerCase();
              return hay.includes(String(v).toLowerCase());
            }),
          );
        }
